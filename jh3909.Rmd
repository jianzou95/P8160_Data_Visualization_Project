...
pollution_aqi_state %>% 
  filter(pollutants == "so2", 
         state == "Nevada"| state =="Iowa"| state =="North Dakota"| state =="Tennessee"| state =="California"|state =="Ohio"|state =="Pennsylvania"|state =="Kansas"|state =="Country Of Mexico"|state =="Indiana") %>%
  mutate(name = forcats::fct_reorder(state, median_aqi_state)) %>%
  ggplot(aes(x = name, y = aqi)) +
  geom_boxplot(outlier.alpha = 0.1) +
  labs(x ="States", y = "Mean aqi for all states", title= "Change of SO2 aqi of all states for 6 years") +
